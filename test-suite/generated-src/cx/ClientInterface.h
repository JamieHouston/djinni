// AUTOGENERATED FILE - DO NOT MODIFY!
// This file generated by Djinni from client_interface.djinni

#pragma once

#include "ClientReturnedRecord.h"
#include <stdint.h>

namespace testsuite {

public interface class IClientInterface
{
public:
    virtual ClientReturnedRecord^ GetRecord (int64_t RecordId, Platform::String^ Utf8string, Platform::IBox<Platform::String>^ Misc);
    virtual double IdentifierCheck (const Platform::Array<uint8_t>^ Data, int32_t R, int64_t Jret);
    virtual Platform::String^ ReturnStr ();
    virtual Platform::String^ MethTakingInterface (IClientInterface^ I);
    virtual Platform::String^ MethTakingOptionalInterface (Platform::IBox<IClientInterface>^ I);
};

}  // namespace testsuite
